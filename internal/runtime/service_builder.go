...
func BuildScanServiceWithContext(ctx context.Context, cfg config.Config) (*api.Service, func() error, error) {
	...
	if oauthClient := (githubconnector.UserOAuthClient{
		ClientID:     cfg.GitHubAppOAuthClientID,
		ClientSecret: cfg.GitHubAppOAuthClientSecret,
	}); oauthClient.Configured() {
		svc.GitHubInstallationVerifier = oauthClient
	}
